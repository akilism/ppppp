...
class Slide0 extends ScanComponent {
    ...
    adjust(last,d){
        ...
        var conti = new Conti(0,0.1,"pct_scroll",function(pct,t){
          t.wave = 0
          t.left = Math.linearTween(pct,-600,center+600,1)
          t.top_alpha = 1
          t.speed = 1
          return t;
        }).abut(0.2,function(pct,t){
          t.wave = Math.linearTween(pct,0,30,1)
          t.speed = Math.linearTween(pct,1,-0.9,1)
          t.left = center
          t.top_alpha = 1
          return t;
        }).abut(0.3,function(pct,t){
          var height = $(window).height() + 100
          t.wave = Math.linearTween(pct,30,-30,1)
          t.speed = Math.linearTween(pct,0.1,0.9,1)
          t.left = center
          t.top_alpha = 1 - pct
          return t;
        }).abut(1,function(pct,t){
    ...
    componentDidMount(){
        ...
        document.getElementById("slide0").appendChild(renderer.view);
      ...
      request.open('GET', "soundtrack.mp3", true);
    ...
    render(){
        return (
            <div>
              <div id="slide1" style={{backgroundImage: "url(parking-lot.jpg)", backgroundSize: "cover", backgroundPositionY: "-283px"}}></div>
              <div id="slide0">
              </div>
            </div>
        )
